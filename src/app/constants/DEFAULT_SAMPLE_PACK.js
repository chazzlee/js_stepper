export const DEFAULT_SAMPLE_PACK = {
  0: {
    label: "Open Hat 1",
    name: "openhat-1",
    path: "./assets/s1/Hihats/open-hihat-1.wav",
  },
  1: {
    label: "Closed Hat 1",
    name: "closedhat-1",
    path: "./assets/s1/Hihats/closed-hihat-1.wav",
  },
  2: {
    label: "Percussion 1",
    name: "percussion-1",
    path: "./assets/s1/Percussion/percussion-9.wav",
  },
  3: {
    label: "Percussion 2",
    name: "percussion-2",
    path: "./assets/s1/Percussion/percussion1.wav",
  },
  4: {
    label: "Percussion 3",
    name: "percussion-3",
    path: "./assets/s1/Percussion/Cymatics x S1 - Percussion 4.wav",
  },
  5: {
    label: "Snare 1",
    name: "snare-1",
    path: "./assets/s1/Snares/Cymatics x S1 - Snare 6.wav",
  },
  6: {
    label: "Snare 2",
    name: "snare-2",
    path: "./assets/s1/Snares/snare8.wav",
  },
  7: { label: "Kick 1", name: "kick-1", path: "./assets/s1/Kicks/kick-5.wav" },
};

export const DEFAULT_SAMPLE_PACK_2 = {
  0: {
    label: "Open Hat 1",
    name: "openhat-1",
    path: "./samples/openhat1.wav",
  },
  1: {
    label: "Closed Hat 1",
    name: "closedhat-1",
    path: "./samples/closedhat1.wav",
  },
  2: {
    label: "Percussion 1",
    name: "percussion-1",
    path: "./samples/perc1.wav",
  },
  3: {
    label: "Percussion 2",
    name: "percussion-2",
    path: "./samples/perc2",
  },
  4: {
    label: "Riff 1",
    name: "riff-1",
    path: "./samples/riff1.wav",
  },
  5: {
    label: "Snare 1",
    name: "snare-1",
    path: "./samples/snare1.wav",
  },
  6: {
    label: "Snare 2",
    name: "snare-2",
    path: "./samples/snare2.wav",
  },
  7: { label: "Kick 1", name: "kick-1", path: "./samples/kick.wav" },
};
